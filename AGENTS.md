...
- HTTP MCP server registrations must preserve the `Http` source kind while using the official MCP C# SDK Streamable HTTP transport and SDK-owned protocol negotiation; expose only supported transport knobs through an options object, and do not expose AutoDetect, SSE, session-resumption compatibility settings, custom-client workarounds, positional-overload sprawl, or hand-written JSON-RPC/SSE transport code.
...
- When a consumer-facing MCP protocol defect is caused by `ManagedCode.MCPGateway` export, routing, or invocation behavior, fix the reusable package and package tests instead of adding app-specific workarounds, because consumers must inherit the corrected MCP contract from the library.
- Gateway tool identity must stay canonical across package surfaces: `McpGatewayToolDescriptor.ToolId`, downstream MCP `Tool.Name`, and `tools/call` names must be the same MCP-safe identifier, while raw upstream `SourceId` and upstream tool name remain separate routing metadata.
- Canonical gateway protocol names for tools, prompts, and exported resource names must be normalized to lowercase MCP-safe identifiers, and gateway-owned name resolution must be case-insensitive because model/tool callers may vary casing such as `ToolID`, `toolid`, or mixed-case tool names.
...
- After adopting a new MCP SDK or protocol baseline, remove every gateway-owned protocol parser, handshake fallback, stateful/session/SSE setting, deprecated capability handler, obsolete model, test, and documentation reference. Leave protocol negotiation to the official SDK unless a host explicitly configures a version, and accept every successfully negotiated SDK client without a second gateway-owned version gate, because the gateway must not contradict a successful handshake.
