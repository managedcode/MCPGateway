...
- `McpGatewaySearchStrategy.Auto` remains a policy mode, but it is not the default and is not a third retrieval engine.
...
- keep `McpGatewaySearchStrategy.Auto` available as a policy mode for hosts that explicitly want adaptive behavior
