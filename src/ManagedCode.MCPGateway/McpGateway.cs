...
public sealed partial class McpGateway : IMcpGateway, IMcpGatewayRegistry
{
    ...
    private readonly McpGatewayRegistry _registry;
    private readonly bool _disposeOwnedRegistry;
        ...
        ILoggerFactory loggerFactory)
    {
        ...
        var registry = serviceProvider.GetService<McpGatewayRegistry>();
        _registry = registry ?? new McpGatewayRegistry(options);
        _disposeOwnedRegistry = registry is null;
    ...
    public void AddHttpServer(
        ...
        => ForwardToRegistry(registry => registry.AddHttpServer(sourceId, endpoint, headers, displayName));
    ...
    public void AddStdioServer(
        ...
        string? displayName = null)
        => ForwardToRegistry(registry => registry.AddStdioServer(sourceId, command, arguments, workingDirectory, environmentVariables, displayName));
    ...
    public void AddMcpClient(
        ...
        string? displayName = null)
        => ForwardToRegistry(registry => registry.AddMcpClient(sourceId, client, disposeClient, displayName));
    ...
    public void AddMcpClientFactory(
        ...
        string? displayName = null)
        => ForwardToRegistry(registry => registry.AddMcpClientFactory(sourceId, clientFactory, disposeClient, displayName));

    public async ValueTask DisposeAsync()
    {
        var disposeOwnedRegistry = false;
        lock (_stateGate)
        {
            if (_disposed)
            {
            ...
            _snapshotVersion = -1;
            disposeOwnedRegistry = _disposeOwnedRegistry;
        }

        if (disposeOwnedRegistry)
        {
    ...
    private void ForwardToRegistry(Action<McpGatewayRegistry> action)
    {
        ThrowIfDisposed();
        action(_registry);
    }
