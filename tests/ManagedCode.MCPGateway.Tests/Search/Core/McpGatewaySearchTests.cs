...
public sealed partial class McpGatewaySearchTests
{
    ...
    private static string SearchGitHubAgain([Description("Search query text.")] string query) =>
        $"github-duplicate:{query}";
    ...
    private static string FilterAdvisories(
        [Description("Severity filter to apply to advisory lookups.")] string severity
    ) => $"advisory:{severity}";
}
