...
public sealed partial class McpGatewaySearchTests
{
    ...
    public async Task SearchAsync_AutoStrategyUsesVectorPrimaryRankingBeforeGraphSupplements()
    {
        ...
        await Assert
            .That(
                searchResult.Diagnostics.Any(static diagnostic =>
                    diagnostic.Code == "hybrid_vector_merge_used"
                )
            )
            .IsFalse();
        await Assert
        ...
        await Assert
            ...
            .IsEqualTo("weather forecast by city");
    }
    ...
    {
        var embeddingGenerator = CreateAutoSemanticEmbeddingGenerator(
            returnZeroVectorOnQuery: static value =>
                string.Equals(value, "weather forecast by city", StringComparison.Ordinal)
        );
        await using var serviceProvider = GatewayTestServiceProviderFactory.Create(
            ConfigureAutoSearchTools,
            embeddingGenerator
    ...
    public async Task SearchAsync_AutoStrategyPreservesOriginalAndNormalizedQueryForMultilingualVectorRanking()
    {
        ...
        await Assert
            .That(embeddingGenerator.Calls[1].Single())
            .Contains("а що у мен з нотіфікешенми");
        await Assert
            .That(embeddingGenerator.Calls[1].Single())
    ...
    private static TestEmbeddingGenerator CreateAutoSemanticEmbeddingGenerator(
        Func<string, bool>? returnZeroVectorOnQuery = null
    ) =>
        new(
