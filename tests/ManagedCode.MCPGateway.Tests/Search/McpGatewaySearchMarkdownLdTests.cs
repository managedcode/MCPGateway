...
{
    private static readonly string[] ReleaseWorkflowAliases = ["релізи", "деплої"];

    private static readonly string[] ReleaseWorkflowKeywords = ["approvals", "merge trains"];
        ...
        await using var serviceProvider = GatewayTestServiceProviderFactory.Create(options =>
        {
            options.AddTool(
                ...
                new McpGatewayToolSearchHints(
                    Aliases: ["сповіщення", "нотифікації", "уведомления"],
    ...
    public async Task SearchAsync_FallsBackToOriginalQueryWhenNormalizationFails()
    {
        var chatClient = new TestChatClient(
            new TestChatClientOptions
            {
                ThrowOnInput = static query =>
                    query.Contains("demande de fusion", StringComparison.Ordinal),
            }
        );
        ...
        var searchResult = await gateway.SearchAsync(
            "demande de fusion pour le depot managedcode",
            maxResults: 1
        );
        ...
        await Assert
            .That(
                searchResult.Diagnostics.Any(static diagnostic =>
                    diagnostic.Code == "query_normalization_failed"
                )
            )
            .IsTrue();
        await Assert
        ...
        var searchResult = await gateway.SearchAsync(
            new McpGatewaySearchRequest(
                ...
                Context: new Dictionary<string, object?>
                {
                    ...
                    ["intent"] = new JsonObject { ["category"] = "weather", ["mode"] = "lookup" },
                    ["compatibility"] = new Hashtable { ["location"] = "Paris", ["active"] = true },
    ...
    public async Task SearchAsync_FallsBackWhenQueryEmbeddingFails()
    {
        var embeddingGenerator = new TestEmbeddingGenerator(
            ...
            {
                ThrowOnInput = static input =>
                    input.Contains("explode query", StringComparison.Ordinal),
            }
        );
        ...
        await Assert
            .That(
                searchResult.Diagnostics.Any(static diagnostic =>
                    diagnostic.Code == "vector_search_failed"
                )
            )
            .IsTrue();
    }
    ...
    public async Task SearchAsync_FallsBackWhenQueryVectorIsEmpty()
    {
        var embeddingGenerator = new TestEmbeddingGenerator(
            new TestEmbeddingGeneratorOptions
            {
                ReturnZeroVectorOnInput = static input =>
                    input.Contains("empty query vector", StringComparison.Ordinal),
            }
        );
        ...
        await Assert
            .That(
                searchResult.Diagnostics.Any(static diagnostic =>
                    diagnostic.Code == "query_vector_empty"
                )
            )
            .IsTrue();
    }
    ...
    private static void ConfigureDefaultMarkdownLdSearchTools(McpGatewayOptions options)
    {
        ...
        options.AddTool(
            ...
            TestFunctionFactory.CreateFunction(
                SearchWeather,
                "weather_air_quality_lookup",
                "Lookup air quality index, smoke exposure, and pollution levels for a location."
            )
        );
        options.AddTool(
            "local",
            TestFunctionFactory.CreateFunction(
                ...
                "Track shipment status, carrier events, and delivery estimates."
            )
        );
        options.AddTool(
            "local",
            TestFunctionFactory.CreateFunction(
                ...
                "Find invoices by customer, invoice number, payment state, or due date."
            )
        );
        options.AddTool(
            "local",
            TestFunctionFactory.CreateFunction(
                SearchWeather,
                "crm_contact_search",
                "Find CRM contacts by name, email, title, account, or segment."
            )
        );
    }
    ...
    private static void ConfigureTravelMarkdownLdTools(McpGatewayOptions options)
    {
        options.AddTool(
            ...
            TestFunctionFactory.CreateFunction(
                SearchWeather,
                "travel_hotel_search",
                "Find hotels by city, district, amenities, breakfast, or cancellation policy."
            )
        );
        options.AddTool(
            "local",
            TestFunctionFactory.CreateFunction(
                ...
                "Build a travel itinerary with flights, stays, meetings, and transfer timing."
            )
        );
        options.AddTool(
            "local",
            TestFunctionFactory.CreateFunction(
                SearchWeather,
                "travel_booking_lookup",
                "Lookup booking confirmation details, ticket numbers, and reservation status."
            )
        );
    }

    private static void ConfigureMarkdownLdSearchToolsForNormalizationFallback(
    ...
    )
    {
        options.AddTool(
            ...
            TestFunctionFactory.CreateFunction(
                ...
                "Search GitHub pull requests by repository, reviewer queue, review approvals, branch, or merge status. Aliases: merge request, demande de fusion."
            )
        );
        options.AddTool(
            "local",
            TestFunctionFactory.CreateFunction(
                SearchGitHub,
                "github_code_search",
                "Search GitHub source code for files, symbols, snippets, or API usages inside repositories."
            )
        );
    }
