...
public sealed partial class McpGatewaySearchTests
{
    ...
    public async Task SearchAsync_MarkdownLdGraphRanksNormalizedNotificationToolAheadOfFamilyRelationshipTools()
    {
        ...
        await Assert.That(searchResult.RankingMode).IsEqualTo("graph");
        await Assert.That(searchResult.Diagnostics.Any(static diagnostic => diagnostic.Code == "low_confidence_results")).IsFalse();
        ...
        await Assert.That(searchResult.Matches.All(static match => match.ToolId != "local:storied_person_add_father" || match.Score < 1d)).IsTrue();
    }
    ...
    public async Task SearchAsync_MarkdownLdGraphKeepsTypoHeavyRelevantMatchesAboveConfidenceThreshold()
    {
        ...
        await Assert.That(searchResult.RankingMode).IsEqualTo("graph");
        await Assert.That(searchResult.Diagnostics.Any(static diagnostic => diagnostic.Code == "low_confidence_results")).IsFalse();
        await Assert.That(searchResult.Matches[0].ToolId).IsEqualTo("local:commerce_shipping_tracking");
        await Assert.That(searchResult.Matches[0].Score).IsGreaterThan(0.5d);
    }

    private static TestChatClient CreateNotificationsRewriteClient()
        => new(new TestChatClientOptions
        {
            RewriteQuery = static query => query.Contains("нотіфік", StringComparison.Ordinal)
    ...
    {
        options.AddTool("local", TestFunctionFactory.CreateFunction(SearchWeather, "storied_person_add_father", "Add a father relationship for a person in a tree."));
        options.AddTool("local", TestFunctionFactory.CreateFunction(SearchWeather, "storied_person_add_mother", "Add a mother relationship for a person in a tree."));
        options.AddTool("local", TestFunctionFactory.CreateFunction(SearchWeather, "storied_person_add_potential_parent", "Apply a suggested potential parent link to a person in a tree."));
    }
    ...
    private static void ConfigureNotificationAndFamilyTools(McpGatewayOptions options)
    {
        ...
        options.AddTool("local", TestFunctionFactory.CreateFunction(SearchWeather, "notification_activity_search", "List notification inbox alerts, unread activity, mentions, and message updates for the current user."));
    }
}
