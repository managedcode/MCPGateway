...
public sealed class McpGatewayMcpServerIntegrationTests
{
    ...
    public async Task ListToolsAsync_ExposesAggregatedToolsFromMultipleUpstreamServers()
    {
        ...
        await Assert.That(tools.Any(static tool => tool.Name == "source-a:github_repository_search"))
            .IsTrue();
        await Assert.That(tools.Any(static tool => tool.Name == "source-b:story_item_detail"))
            .IsTrue();
        await Assert.That(tools.Any(static tool => tool.Name == "source-c:incident_status_lookup"))
            .IsTrue();
    }
    ...
    public async Task ListPromptsAsync_ExposesAggregatedPromptsFromMultipleUpstreamServers()
    {
        ...
        await Assert.That(prompts.Any(static prompt =>
            ...
            .IsTrue();
        await Assert.That(prompts.Any(static prompt => prompt.Name == "source-b:story_triage_system_prompt"))
            .IsTrue();
        await Assert.That(prompts.Any(static prompt =>
